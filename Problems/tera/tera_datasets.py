...
#
#             print ">>>>>>", p(pd), p(pf), p(prec)
#
#
#         return p(pd), p(pf), p(prec) #return just the last row (bugs found class)
#
#
...
def evaluator(input, properties):
    ...
    if properties.type == "default":
        ...
        clf = DecisionTreeRegressor(max_features=mxf, min_samples_split=mss, min_samples_leaf=msl, random_state= 1)
...
class ant(jmoo_problem):  # ant 15 is the latest, it can't see anything other than 1.5
    def __init__(prob):
        prob.name = "ant"
        prob.decisions = tera_decisions
        prob.objectives = tera_objectives
